...
def contrFin1Fin1 {n : ℕ} (c : Fin n.succ.succ → S.C)
    ...
    S.F.obj (OverColor.mk ((c ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inl)) ≅
    (OverColor.Discrete.pairτ S.FD S.τ).obj { as := c i } := by
  apply (S.F.mapIso
    (OverColor.mkSum (((c ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inl)))).trans
...
lemma contrFin1Fin1_inv_tmul {n : ℕ} (c : Fin n.succ.succ → S.C)
  ...
  change ((OverColor.lift.obj S.FD).map (OverColor.mkSum
    ((c ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inl)).inv).hom
  ...
  | Sum.inl 0 =>
    simp only [Nat.succ_eq_add_one, mk_hom, Fin.isValue, Function.comp_apply,
      ...
      Sum.elim_inr, PhysLean.PiTensorProduct.elimPureTensor]
  ...
  | Sum.inr 0 =>
    simp only [Nat.succ_eq_add_one, mk_hom, Fin.isValue, Function.comp_apply,
      ...
      Sum.elim_inl, Sum.elim_inr, PhysLean.PiTensorProduct.elimPureTensor,
...
def contrIso {n : ℕ} (c : Fin n.succ.succ → S.C)
  ...
  (S.F.mapIso (OverColor.equivToIso (PhysLean.Fin.finExtractTwo i j))).trans <|
  (S.F.mapIso (OverColor.mkSum (c ∘ (PhysLean.Fin.finExtractTwo i j).symm))).trans <|
...
lemma contrIso_hom_hom {n : ℕ} {c1 : Fin n.succ.succ → S.C}
    ...
    (S.F.map (equivToIso (PhysLean.Fin.finExtractTwo i j)).hom).hom ≫
    (S.F.map (mkSum (c1 ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm)).hom).hom ≫
    (Functor.Monoidal.μIso S.F
    (OverColor.mk ((c1 ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inl))
    (OverColor.mk ((c1 ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inr))).inv.hom ≫
...
lemma contrMap_tprod {n : ℕ} (c : Fin n.succ.succ → S.C)
  ...
  change (λ_ ((lift.obj S.FD).obj _)).hom.hom
    ...
    (OverColor.mk ((c ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm) ∘ Sum.inl))
  ...
  change (λ_ ((lift.obj S.FD).obj (OverColor.mk (c ∘ i.succAbove ∘ j.succAbove)))).hom.hom
    ...
    ((Hom.toEquiv
    (mkSum (c ∘ ⇑(PhysLean.Fin.finExtractTwo i j).symm)).hom).symm (Sum.inr i_1)))))))) = _
  ...
  · simp only [Fin.isValue, castToField]
    ...
    · simp only [Fin.isValue, lift.discreteFunctorMapEqIso, eqToIso_refl, Functor.mapIso_refl,
      ...
      change (S.FD.map (eqToHom _)).hom
        (x (((PhysLean.Fin.finExtractTwo i j)).symm ((Sum.inl (Sum.inr 0))))) = _
      ...
      have h1' {a b d: Fin n.succ.succ} (hbd : b =d) (h : c d = S.τ (c a)) (h' : c b = S.τ (c a)) :
        ...
        rfl
      refine h1' ?_ ?_ ?_
      simp only [Nat.succ_eq_add_one, Fin.isValue, PhysLean.Fin.finExtractTwo_symm_inl_inr_apply]
      simp [h]
  /- The tensor. -/
  · conv_lhs => erw [lift.map_tprod]
    ...
    have h1 : ((PhysLean.Fin.finExtractTwo i j).symm (Sum.inr d))
      = (i.succAbove (j.succAbove d)) := PhysLean.Fin.finExtractTwo_symm_inr_apply i j d
