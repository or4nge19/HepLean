...
lemma contrMap_prod_tprod_aux
    ...
    (S.FD.map (eqToHom (by
      ...
      rfl))).hom
    ((lift.discreteSumEquiv' S.FD l')
    (PhysLean.PiTensorProduct.elimPureTensor p q' l')) := by
  subst h'
  match l with
  | Sum.inl l =>
...
lemma contrMap_prod_tprod_aux_2 (p : (i : (𝟭 Type).obj (OverColor.mk c).left) →
    ...
    (PhysLean.PiTensorProduct.elimPureTensor p q' b)) := by
  subst h
...
lemma contrMap_prod :
  ...
  exact PhysLean.PiTensorProduct.induction_tmul (fun p q' => q.contrMap_prod_tprod p q')
...
lemma prod_contrMap :
  ...
  ext1
  refine ModuleCat.hom_ext ?_
  exact PhysLean.PiTensorProduct.induction_tmul (fun p q' => q.prod_contrMap_tprod p q')
