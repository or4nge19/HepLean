...
def preContrCoUnit (d : ℕ := 3) : 𝟙_ (Rep ℝ (LorentzGroup d)) ⟶ Contr d ⊗ Co d :=
  CategoryTheory.ConcreteCategory.ofHom (C := Rep ℝ (LorentzGroup d))
    (show (𝟙_ (Rep ℝ (LorentzGroup d))).ρ.IntertwiningMap (Contr d ⊗ Co d).ρ from
     ⟨(LinearMap.id (R := ℝ) (M := ℝ)).smulRight (preContrCoUnitVal d),
      fun M => LinearMap.ext fun x => by
        simp only [LinearMap.coe_comp, Function.comp_apply, LinearMap.coe_smulRight,
          Rep.tensorUnit_ρ, Rep.tensor_ρ, map_smul]
        congr 1
        simp only [preContrCoUnitVal]
        have ht :
            (((Contr d).ρ.tprod (Co d).ρ) M) (contrCoToMatrixRe.symm 1) =
              TensorProduct.map ((Contr d).ρ M) ((Co d).ρ M) (contrCoToMatrixRe.symm 1) := rfl
        calc
          contrCoToMatrixRe.symm 1 =
              contrCoToMatrixRe.symm
                (M.1 * (1 : Matrix (Fin 1 ⊕ Fin d) (Fin 1 ⊕ Fin d) ℝ) * M.1⁻¹) :=
            congrArg _ (by simp)
          _ = TensorProduct.map ((Contr d).ρ M) ((Co d).ρ M) (contrCoToMatrixRe.symm 1) :=
            (contrCoToMatrixRe_ρ_symm (1 : Matrix (Fin 1 ⊕ Fin d) (Fin 1 ⊕ Fin d) ℝ) M).symm
          _ = (((Contr d).ρ.tprod (Co d).ρ) M) (contrCoToMatrixRe.symm 1) := ht.symm
     ⟩)

lemma preContrCoUnit_apply_one {d : ℕ} : (preContrCoUnit d).hom (1 : ℝ) = preContrCoUnitVal d := by
  unfold preContrCoUnit
  simp [Rep.hom_ofHom, LinearMap.smulRight_apply, one_smul]
...
def preCoContrUnit (d : ℕ) : 𝟙_ (Rep ℝ (LorentzGroup d)) ⟶ Co d ⊗ Contr d :=
  CategoryTheory.ConcreteCategory.ofHom (C := Rep ℝ (LorentzGroup d))
    (show (𝟙_ (Rep ℝ (LorentzGroup d))).ρ.IntertwiningMap (Co d ⊗ Contr d).ρ from
     ⟨(LinearMap.id (R := ℝ) (M := ℝ)).smulRight (preCoContrUnitVal d),
      fun M => LinearMap.ext fun x => by
        simp only [LinearMap.coe_comp, Function.comp_apply, LinearMap.coe_smulRight,
          Rep.tensorUnit_ρ, Rep.tensor_ρ, map_smul]
        congr 1
        simp only [preCoContrUnitVal]
        have ht :
            (((Co d).ρ.tprod (Contr d).ρ) M) (coContrToMatrixRe.symm 1) =
              TensorProduct.map ((Co d).ρ M) ((Contr d).ρ M) (coContrToMatrixRe.symm 1) := rfl
        calc
          coContrToMatrixRe.symm 1 =
              coContrToMatrixRe.symm
                (M.1⁻¹ᵀ * (1 : Matrix (Fin 1 ⊕ Fin d) (Fin 1 ⊕ Fin d) ℝ) * M.1ᵀ) :=
            congrArg _ (by
              simp only [Matrix.mul_one]
              rw [← Matrix.transpose_mul (M := M.1) (N := M.1⁻¹), mul_inv_of_invertible M.1,
                Matrix.transpose_one])
          _ = TensorProduct.map ((Co d).ρ M) ((Contr d).ρ M) (coContrToMatrixRe.symm 1) :=
            (coContrToMatrixRe_ρ_symm (1 : Matrix (Fin 1 ⊕ Fin d) (Fin 1 ⊕ Fin d) ℝ) M).symm
          _ = (((Co d).ρ.tprod (Contr d).ρ) M) (coContrToMatrixRe.symm 1) := ht.symm
     ⟩)

lemma preCoContrUnit_apply_one {d : ℕ} : (preCoContrUnit d).hom (1 : ℝ) = preCoContrUnitVal d := by
  unfold preCoContrUnit
  simp [Rep.hom_ofHom, LinearMap.smulRight_apply, one_smul]
...
lemma contr_preContrCoUnit {d : ℕ} (x : Co d) :
  ...
  have h1 : ((α_ (Co d) _ (Co d)).inv.hom (x ⊗ₜ[ℝ] (preContrCoUnit d).hom (1 : ℝ)))
    ...
    rw [preContrCoUnit_apply_one, preContrCoUnitVal_expand_tmul, tmul_sum, map_sum]
    congr 1
  rw [h1]
  have h2 : (coContrContract ▷ (Co d)).hom (∑ i, (x ⊗ₜ[ℝ] contrBasis d i) ⊗ₜ[ℝ] coBasis d i)
    ...
    simp only [Rep.hom_whiskerRight]
    rw [map_sum]
    rfl
  rw [h2]
...
lemma contr_preCoContrUnit {d : ℕ} (x : (Contr d)) :
  ...
  have h1 : ((α_ (Contr d) _ (Contr d)).inv.hom (x ⊗ₜ[ℝ] (preCoContrUnit d).hom (1 : ℝ)))
    ...
    rw [preCoContrUnit_apply_one, preCoContrUnitVal_expand_tmul, tmul_sum, map_sum]
    congr 1
  rw [h1]
